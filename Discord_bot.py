...
class Stream_Sink(voice_recv.AudioSink):
    class Google_drive:
        ...
        def upload_data(self, wav_data):
            ...
            threading.Timer(15, thread_relation().recodedelete_thread,args=(wav_path,)).start()
    ...
    def write(self, user, data):
        ...
        try:
            ...
            currnt_buffer = streamAudio_buffer[self.guild_id][user_id]
            currnt_buffer.append(downsampled_samples.tobytes())

            if len(streamAudio_buffer[self.guild_id][user_id]) * self.chunk * 2 >= self.Rate * 2 * 120:  # 2분 이상의 데이터가 쌓이면 처리
                audio_data = b"".join(currnt_buffer)
                streamAudio_buffer[self.guild_id][user_id] = []
                ...
                try: self.Google_drive(user).upload_data(wav_buffer.getvalue())
                except: print(f"{user.nick}/{user.id} 실패")
        except: print(f"write 실패")
