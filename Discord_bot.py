...
class Stream_Sink(voice_recv.AudioSink):
    class Google_drive:
        def __init__(self,user,guild_id):
            self.parent_folder_id = parent_folder_id # 구글드라이브 조상폴더 위치
            self.user = user
            self.guild_id = guild_id
            self.service = self._authenticate()
        def _authenticate(self):
            creds = None
            storage_path = os.path.dirname(os.path.abspath(__file__)) + "/google_key.json"
            token_path = os.path.dirname(os.path.abspath(__file__)) + "/token.pickle"
            if os.path.exists(token_path):
                with open(token_path, 'rb') as token:
                    creds = pickle.load(token)
            # 사용 가능한 자격 증명이 없으면 사용자를 로그인하게 함
        ...
        def _create_or_get_folder_id(self, folder_name):
            results = self.service.files().list(
                q=f"mimeType='application/vnd.google-apps.folder' and name='{folder_name}' and '{self.parent_folder_id}' in parents",
                fields="files(id)"
            ).execute()
        ...
        def upload_data(self, wav_data):
            folder_name = datetime.now().strftime("%Y-%m-%d") + f"({self.user.guild.name})({self.user.guild.id})"
            today_folder_id = self._create_or_get_folder_id(folder_name)
            file_name = f"({self.user.nick})({self.user.id})" + datetime.now().strftime("%HH%MM%SS") + ".wav"
            file_metadata = {'name': file_name, 'parents': [today_folder_id]}
            
            wav_path = os.path.dirname(os.path.abspath(__file__)) + "/stts" + f"/wav_data({self.user.id}).wav"
            ...
            print(f"{self.user.nick}/{self.user.id} 녹음")
            try: threading.Timer(10, thread_relation().wavdelete_thread,args=(wav_path,)).start()
            except: pass
    ...
    def __init__(self,guild_id) -> None:
        ...
        self.guild_id = guild_id
        self.Rate = 24000
        self.chunk = (self.Rate // 1000)*20
        
    def write(self, user, data):
        global streamAudio_buffer,Ratequeue
        try:
            user_id = user.id
            downsampled_samples = np.frombuffer(data.pcm, dtype=np.int16)[::4]
            current_buffer = None

            if user_id not in streamAudio_buffer[self.guild_id]:
                print("녹음 시작:",user.nick)
                streamAudio_buffer[self.guild_id][user_id] = []

            current_buffer = streamAudio_buffer[self.guild_id][user_id]
            current_buffer.append(downsampled_samples.tobytes())

            if len(streamAudio_buffer[self.guild_id][user_id]) * self.chunk * 2 >= self.Rate * 2 * 120:  # 2분 이상의 데이터가 쌓이면 처리
                audio_data = b"".join(current_buffer)
                Ratequeue.append([audio_data,user])
                streamAudio_buffer[self.guild_id][user_id] = []
                stream_channel[self.guild_id][1].stop()
                stream_channel[self.guild_id][0].listen(Stream_Sink(self.guild_id))
                if len(Ratequeue)==1 and not Google_driveRunning: self.Google_drive_queue()
        except: 
            print(f"write 실패")
    ...
    def Google_drive_queue(self):
        global Google_driveRunning,Ratequeue
        Google_driveRunning = True
        queue_data = Ratequeue.pop(0)
        wav_buffer = io.BytesIO()
        with wave.open(wav_buffer, 'wb') as wf:
            wf.setnchannels(1)
            wf.setsampwidth(pyaudio.get_sample_size(pyaudio.paInt16))
            wf.setframerate(self.Rate)
            wf.writeframes(queue_data[0])
        try: self.Google_drive(queue_data[1],self.guild_id).upload_data(wav_buffer.getvalue())
        except: print(f"{queue_data[1].nick}/{queue_data[1].id} 실패")
        
        if len(Ratequeue)==0: Google_driveRunning = False
        else: self.Google_drive_queue()
    ...
    if message.channel.id == guild_Dict[guild_id][0]:
        ...
        if message.author.voice:
            if client.voice_clients == []:
                if message.content[0:2] == '??':#375969442338701322:
                    stream_channel[guild_id][0] = await message.author.voice.channel.connect(cls=voice_recv.VoiceRecvClient)
                    stream_channel[guild_id][0].listen(Stream_Sink(guild_id))
                    print("스트리밍")
